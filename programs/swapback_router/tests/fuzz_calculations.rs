...
mod fuzz_tests {
    ...
    fn fuzz_platform_fee_no_overflow() {
        ...
        for amount in test_values {
            ...
            let fee = amount.checked_mul(PLATFORM_FEE_BPS)
    ...
    fn fuzz_npi_distribution_no_overflow() {
        ...
        let npi_values: Vec<u64> = vec![
            0, 1, 100, 1_000, 10_000, 100_000,
            1_000_000, 10_000_000, 100_000_000,
            u64::MAX / 10_000,
        ];

        for npi in npi_values {
            let rebate = npi.checked_mul(DEFAULT_REBATE_BPS)
                .and_then(|v| v.checked_div(10_000));
            let treasury = npi.checked_mul(TREASURY_FROM_NPI_BPS)
                .and_then(|v| v.checked_div(10_000));
            let boost = npi.checked_mul(BOOST_VAULT_BPS)
                .and_then(|v| v.checked_div(10_000));
    ...
    fn fuzz_slippage_bounds() {
        ...
        let amount_values: Vec<u64> = vec![
            0, 1_000_000, 100_000_000, 10_000_000_000, 
            100_000_000_000, u64::MAX / 1000
        ];
        ...
        for volatility in &volatility_values {
            for amount in &amount_values {
                ...
                assert!(slippage <= max_bps, 
                    "Slippage {} exceeded max {} for vol={}, amt={}", 
                    slippage, max_bps, volatility, amount);
                assert!(slippage >= base_bps.min(max_bps),
                    "Slippage {} below min {} for vol={}, amt={}",
                    slippage, base_bps.min(max_bps), volatility, amount);
            }
        }
    }
    ...
    fn fuzz_dca_slice_distribution() {
        ...
        for &total in &amounts {
            for &slices in &slice_counts {
                ...
                assert!(remainder < slices as u64,
                    "Remainder {} >= slices {} for total={}", remainder, slices, total);
                
                // Total from slices should never exceed original
                assert!(total_from_slices <= total,
                    "Slices total {} > original {} for slices={}", 
                    total_from_slices, total, slices);
            }
        }
    }
        ...
        for &interval in &intervals {
            for &slices in &slice_counts {
                ...
                for i in 1..expected_times.len() {
                    assert!(expected_times[i] > expected_times[i-1],
                ...
                for i in 1..expected_times.len() {
                    ...
                    assert_eq!(gap, interval as i64,
                        "Interval mismatch at index {}", i);
                }
            }
        }
    }
    ...
    fn fuzz_venue_score_bounds() {
        ...
        fn calculate_score(v: &MockVenue) -> u16 {
            ...
            let slippage_score = ((10000u16.saturating_sub(v.slippage)) as f64 * 0.3).min(3000.0) as u16;
            ...
            for &npi in &npis {
                for &lat in &latencies {
                    for &slip in &slippages {
                        let venue = MockVenue { volume: vol, npi, latency: lat, slippage: slip };
                        let score = calculate_score(&venue);
                        
                        assert!(score <= 10000,
                            "Score {} > 10000 for vol={}, npi={}, lat={}, slip={}",
                            score, vol, npi, lat, slip);
                    }
                }
            }
        }
        ...
        for &amount in &amounts {
            for &tvl in &tvls {
                for &price in &prices {
                    ...
                    let expected_output = amount.checked_mul(price)
    ...
    fn fuzz_minimum_amounts() {
        ...
        let amounts: Vec<u64> = vec![
            0, 1, 10, 100, 999, 1000, 1001, 10_000, 100_000, 1_000_000,
        ];
    ...
    fn fuzz_calculation_determinism() {
        ...
        for (amount, npi, slippage) in test_cases {
            ...
            assert_eq!(results.len(), 1,
                "Non-deterministic result for amount={}", amount);
        }
    }
    ...
    fn fuzz_rounding_always_down() {
        ...
        let numerators: Vec<u64> = vec![
            1, 5, 9, 10, 11, 99, 100, 101, 999, 1000, 1001,

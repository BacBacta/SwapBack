...
export function TransactionTracker() {
  ...
  const currentStepIndex = steps.findIndex((step) => step.key === transaction.status);
            ...
            <div className="mt-4 p-4 bg-red-900 bg-opacity-20 border border-red-500 rounded-xl">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  <h4 className="text-red-500 font-semibold mb-1">Transaction Failed</h4>
          ...
          {transaction.status === 'confirmed' && (
            <div className="mt-4 p-4 bg-green-900 bg-opacity-20 border border-green-500 rounded-xl">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1">
                  <h4 className="text-green-500 font-semibold mb-1">Swap Successful!</h4>
                  <p className="text-sm text-green-300">Your transaction has been finalized</p>
                </div>
              </div>
              <button
                onClick={resetTransaction}
        ...
        <div>
          ...
          <div className="space-y-2">
            {transactionHistory.slice(0, 10).map((tx, index) => (
              <div key={tx.signature} className="bg-gray-800 rounded-lg p-3 hover:bg-gray-750">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className={tx.status === 'confirmed' ? 'text-green-500' : 'text-red-500'}>
                      {tx.status === 'confirmed' ? '✓' : '✗'}
                    </span>
                    <div>
                      <div className="text-sm text-white">
                        {tx.inputAmount} {tx.inputToken.symbol} → {tx.outputAmount}{' '}

...
export class RouteOptimizationEngine {
  ...
  async findOptimalRoutes(
    ...
    // 3c. Multi-hop routes (native discovery through intermediate tokens)
    if (config.maxHops && config.maxHops >= 2) {
      try {
        const multiHopCandidates = await this.findMultiHopRoutes(
          inputMint,
          outputMint,
          inputAmount,
          config
        );
        candidates.push(...multiHopCandidates);
      } catch (error) {
        this.logger.warn("multi_hop_discovery_failed", {
   ...
   * Prioritized by liquidity depth on Solana
   */
  private static readonly INTERMEDIATE_TOKENS: { mint: string; symbol: string }[] = [
   ...
   * Explores paths like: INPUT → USDC → OUTPUT, INPUT → SOL → OUTPUT
   */
  async findMultiHopRoutes(
    ...
    // Skip if multi-hop is disabled or max hops is 1
    if (maxHops < 2) {
      return candidates;
    }

    // Filter out intermediate tokens that are the same as input or output
    const viableIntermediates = RouteOptimizationEngine.INTERMEDIATE_TOKENS.filter(
      (token) => token.mint !== inputMint && token.mint !== outputMint
    );

    this.logger.debug("multi_hop_discovery_start", {
      inputMint,
      outputMint,
      inputAmount,
      intermediateCount: viableIntermediates.length,
    });

    // Try each intermediate token as a hop
    const hopPromises = viableIntermediates.map(async (intermediate) => {
      try {
        return await this.evaluateMultiHopPath(
          inputMint,
          intermediate.mint,
          outputMint,
          inputAmount,
        ...
        this.logger.warn("multi_hop_path_failed", {
    ...
    const results = await Promise.all(hopPromises);
    ...
    this.logger.info("multi_hop_routes_found", {
      count: candidates.length,
      bestOutput: candidates[0]?.expectedOutput ?? 0,
    });
  ...
  /**
   * Evaluate a specific multi-hop path: INPUT → INTERMEDIATE → OUTPUT
   */
  private async evaluateMultiHopPath(
    ...
    // Step 1: Get liquidity for first leg (INPUT → INTERMEDIATE)
    const firstLegLiquidity = await this.liquidityCollector.fetchAggregatedLiquidity(
      inputMint,
      intermediateMint,
      inputAmount
    );

    if (firstLegLiquidity.sources.length === 0) {
      return null;
    }

    // Use best source for first leg
    const firstLegSource = firstLegLiquidity.sources[0];
    const intermediateAmount = this.calculateExpectedOutput(firstLegSource, inputAmount);

    if (intermediateAmount <= 0) {
      return null;
    }

    // Step 2: Get liquidity for second leg (INTERMEDIATE → OUTPUT)
    const secondLegLiquidity = await this.liquidityCollector.fetchAggregatedLiquidity(
      intermediateMint,
      outputMint,
      intermediateAmount
    );

    if (secondLegLiquidity.sources.length === 0) {
      return null;
    }

    // Use best source for second leg
    const secondLegSource = secondLegLiquidity.sources[0];
    const finalOutput = this.calculateExpectedOutput(secondLegSource, intermediateAmount);
    ...
    const totalSlippage = firstLegSource.slippagePercent + secondLegSource.slippagePercent;
    
    // Reject if total slippage exceeds tolerance
    if (totalSlippage > config.slippageTolerance) {
      return null;
    }

    // Build the multi-hop route
    const firstSplit: RouteSplit = {
      venue: firstLegSource.venue,
      weight: 100,
      inputAmount,
      expectedOutput: intermediateAmount,
      liquiditySource: firstLegSource,
    };

    const secondSplit: RouteSplit = {
      venue: secondLegSource.venue,
      weight: 100,
      inputAmount: intermediateAmount,
      expectedOutput: finalOutput,
      liquiditySource: secondLegSource,
    };

    const route: RouteCandidate = {
      id: `multihop-${intermediateSymbol}-${Date.now()}`,
      venues: [firstLegSource.venue, secondLegSource.venue],
      path: [inputMint, intermediateMint, outputMint],
      hops: 2,
      splits: [firstSplit, secondSplit],
      expectedOutput: finalOutput,
      totalCost: this.estimateMultiHopCost(firstLegSource, secondLegSource),
      effectiveRate: finalOutput / inputAmount,
      riskScore: this.assessMultiHopRisk(firstLegSource, secondLegSource),
      mevRisk: "high", // Multi-hop has higher MEV risk
      instructions: [],
      estimatedComputeUnits: this.estimateComputeUnits(VenueType.AMM, 2) * 1.5, // 50% more CU for 2 hops
    };

    this.logger.debug("multi_hop_path_evaluated", {
      path: `${inputMint} → ${intermediateSymbol} → ${outputMint}`,
      intermediateAmount,
      finalOutput,
      totalSlippage,
    });
  ...
  /**
   * Estimate cost for multi-hop route (gas + fees)
   */
  private estimateMultiHopCost(
    firstLeg: LiquiditySource,
    secondLeg: LiquiditySource
  ): number {
    // Base cost for each hop
    const baseCostPerHop = 0.0001; // ~100 micro SOL
    
    // Add venue-specific fees
    const firstLegFee = firstLeg.feeAmount ?? 0;
    const secondLegFee = secondLeg.feeAmount ?? 0;
    
    return baseCostPerHop * 2 + firstLegFee + secondLegFee;
  }

  /**
   * Assess risk for multi-hop route
   */
  private assessMultiHopRisk(
    firstLeg: LiquiditySource,
    secondLeg: LiquiditySource
  ): number {
    // Base risk for multi-hop
    let risk = 30;
    
    // Add risk for low liquidity
    const avgDepth = (firstLeg.depth + secondLeg.depth) / 2;
    if (avgDepth < 10000) risk += 20;
    else if (avgDepth < 100000) risk += 10;
    
    // Add risk for high slippage
    const avgSlippage = (firstLeg.slippagePercent + secondLeg.slippagePercent) / 2;
    if (avgSlippage > 0.02) risk += 15;
    else if (avgSlippage > 0.01) risk += 5;

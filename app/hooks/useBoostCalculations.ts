...
export function useBoostCalculations() {
   ...
   */
  const calculateBoost = useCallback((amount: number, durationDays: number): BoostCalculation => {
  ...
  const estimateAPY = useCallback((
    ...
    const multiplier = (10_000 + boostBP) / 10_000;
    const monthlyRebateRevenue = monthlySwapVolume * baseRebateRate * (multiplier - 1);
   ...
   */
  const getRebateExamples = useCallback((baseRebate: number = 3) => [
    {
      scenario: 'Sans boost',
...
export function getLevelBenefits() {
  return {
    ...
    gold: {
      ...
      icon: '🥇',
      maxBoost: '85%',
      benefits: ['Rebates boostés', 'Distribution buyback', 'Badge spécial', 'Accès prioritaire'],
    },
    platinum: {
      name: 'Platinum',
      icon: '💎',
      maxBoost: '95%',
      benefits: ['Rebates boostés', 'Distribution buyback', 'Badge spécial', 'Accès prioritaire', 'Gouvernance'],
    },
    diamond: {
      name: 'Diamond',
      icon: '💎',
      maxBoost: '100%',
      benefits: ['Rebates boostés max', 'Distribution buyback max', 'Badge exclusif', 'Accès VIP', 'Gouvernance premium'],
    },
  };
}

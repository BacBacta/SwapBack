...
const createMockPythAccountData = (price: number, confidence: number, exponent = -8): Buffer => {
...
const createMockPythPriceData = (price: number, confidence: number, exponent = -8) => ({
...
describe('OraclePriceService', () => {
  ...
  describe('Pyth Price Fetching', () => {
    ...
    it('should validate price freshness and reject stale prices', async () => {
      ...
      await expect(service.getTokenPrice(mockMint)).rejects.toThrow(/No oracle price available/);
    });
  ...
  describe('Switchboard Fallback', () => {
    it('should fallback to Switchboard when Pyth fails', async () => {
      ...
      switchboardBuffer.writeBigInt64LE(BigInt(Math.floor(Date.now() / 1000)), 272); // timestamp at offset 272
  ...
  describe('Route Price Verification', () => {
    it('should accept route when price is within deviation threshold', async () => {
      ...
      mockConnection.getAccountInfo
        ...
        .mockResolvedValueOnce({ ...mockAccountInfo, data: createMockPythAccountData(1, 0.001) }); // USDC
    ...
    it('should reject route when price deviates too much', async () => {
      ...
      mockConnection.getAccountInfo
        ...
        .mockResolvedValueOnce({ ...mockAccountInfo, data: createMockPythAccountData(1, 0.001) });
  ...
  describe('Error Handling', () => {
    it('should handle network errors gracefully', async () => {
      ...
      mockConnection.getAccountInfo.mockRejectedValue(new Error('Network timeout'));
    ...
    it('should handle corrupted price data', async () => {
      ...
      mockConnection.getAccountInfo.mockResolvedValueOnce(corruptedSwitchboardData);

...
if (!RUN_ANCHOR_TESTS) {
  console.warn(
    "⏭️  Skip TODO #1 - Initialize State PDA (set SWAPBACK_RUN_ANCHOR_TESTS=true to enable)."
  );
}
...
describeAnchor("TODO #1 - Initialize State PDA", () => {
  it("should initialize Router State PDA on devnet", async () => {
    ...
    console.log(
      `   Explorer: https://explorer.solana.com/address/${statePda.toBase58()}?cluster=devnet\n`
    );
  }, 60000);
});

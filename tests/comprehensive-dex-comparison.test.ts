...
async function getJupiterQuote(
  ...
  try {
    ...
    if (!response.ok) {
      console.log(
        `⚠️  Jupiter: Paire ${inputMint.slice(0, 8)}... → ${outputMint.slice(0, 8)}... non disponible`
      );
 ...
 */
function getRaydiumQuote(inputAmount: number): {
  output: number;
  fees: number;
  available: boolean;
} {
 ...
 */
function getOrcaQuote(inputAmount: number): {
  output: number;
  fees: number;
  available: boolean;
} {
...
async function testPair(
  ...
  console.log(
    `   Jupiter:  ${jupiter.available ? jupiter.output.toFixed(6) : "N/A"}`
  );
  console.log(`   Raydium:  ${raydium.output.toFixed(6)}`);
  console.log(`   Orca:     ${orca.output.toFixed(6)}`);
  console.log(
    `   SwapBack: ${swapback.available ? swapback.output.toFixed(6) : "N/A"} ${winner.name === "SwapBack" ? "🥇" : ""}`
  );
...
async function runComprehensiveTests(): Promise<TestResult[]> {
  ...
  console.log(
    `SwapBack gagnant: ${swapbackWins}/${successCount} (${((swapbackWins / successCount) * 100).toFixed(1)}%)`
  );
  ...
  const avgAdvantage =
    results
      ...
      .reduce((sum, r) => sum + r.swapbackAdvantage, 0) /
    results.filter((r) => r.swapback.available).length;
  ...
  results
    ...
    .forEach((r, i) => {
      console.log(
        `   ${i + 1}. ${r.pair}: +${r.swapbackAdvantage.toFixed(3)}% (${r.inputAmount} tokens)`
      );
    });
...
describe("Comprehensive DEX Comparison", () => {
  it("should compare SwapBack routes against Jupiter, Raydium, and Orca on 50 token pairs", async () => {
    ...
    console.log(
      `   SwapBack gagnant: ${swapbackWins}/${successfulTests.length}`
    );

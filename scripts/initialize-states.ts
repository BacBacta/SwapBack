...
async function main() {
  ...
  console.log(
    "║         🔧 INITIALISATION DES STATES - SYSTÈME DE BOOST          ║"
  );
  console.log(
    "║                                                                   ║"
  );
  ...
  if (fs.existsSync(deployedIdsPath)) {
    ...
    console.log(
      `${colors.green}✅ Program IDs chargés depuis deployed-program-ids.json${colors.reset}\n`
    );
  } else {
    console.log(
      `${colors.yellow}⚠️  Fichier deployed-program-ids.json non trouvé${colors.reset}`
    );
    console.log(
      `${colors.yellow}Utilisation des IDs par défaut depuis Anchor.toml${colors.reset}\n`
    );
  ...
  if (fs.existsSync(walletPath)) {
    ...
    console.log(
      `${colors.green}✅ Wallet chargé: ${wallet.publicKey.toString()}${colors.reset}`
    );
  } else {
    console.log(
      `${colors.red}❌ Wallet non trouvé à ${walletPath}${colors.reset}`
    );
    console.log(
      `${colors.yellow}Création d'un nouveau wallet...${colors.reset}`
    );
    ...
    console.log(
      `${colors.green}✅ Nouveau wallet créé et sauvegardé${colors.reset}`
    );
  }
  ...
  if (balance < 0.1 * 1e9) {
    console.log(
      `${colors.yellow}⚠️  Solde insuffisant pour l'initialisation${colors.reset}`
    );
    console.log(
      `${colors.yellow}Demandez un airdrop avec: solana airdrop 2${colors.reset}\n`
    );
    process.exit(1);
  }
  ...
  console.log(
    `${colors.blue}1️⃣  Initialisation de GlobalState (swapback_cnft)...${colors.reset}`
  );
  ...
  try {
    ...
    try {
      ...
      console.log(
        `   ${colors.yellow}⚠️  GlobalState déjà initialisé${colors.reset}`
      );
      console.log(
        `   Total Community Boost: ${existingState.totalCommunityBoost}`
      );
      console.log(`   Active Locks: ${existingState.activeLocksCount}`);
    } catch (error) {
      // Pas encore initialisé, on l'initialise
      console.log(
        `   ${colors.yellow}Envoi de la transaction...${colors.reset}`
      );
      ...
      console.log(
        `   ${colors.green}✅ GlobalState initialisé !${colors.reset}`
      );
  ...
  console.log(
    `${colors.blue}2️⃣  Initialisation de RouterState (swapback_router)...${colors.reset}`
  );
  ...
  try {
    ...
    try {
      ...
      console.log(
        `   ${colors.yellow}⚠️  RouterState déjà initialisé${colors.reset}`
      );
    } catch (error) {
      console.log(
        `   ${colors.yellow}Envoi de la transaction...${colors.reset}`
      ...
      console.log(
        `   ${colors.green}✅ RouterState initialisé !${colors.reset}`
      );
      console.log(`   Transaction: ${tx}`);
      console.log(
        `   Explorer: https://explorer.solana.com/tx/${tx}?cluster=devnet\n`
  ...
  console.log(
    `${colors.blue}3️⃣  Initialisation de BuybackState (swapback_buyback)...${colors.reset}`
  );

  try {
    const buybackProgramId = new PublicKey(
      programIds.programs.swapback_buyback
    );
  ...
  console.log(
    "  ⚠️  BuybackState (swapback_buyback) - nécessite configuration manuelle\n"
  );

...
async function testJupiterQuote() {
  ...
  const amount = process.env.AMOUNT ? parseFloat(process.env.AMOUNT) * 1e9 : 0.1 * 1e9; // Default: 0.1 SOL
  ...
  try {
    ...
    console.log('   Effective Price:', effectivePrice.toFixed(4), 'USDC per SOL\n');
    ...
    if (process.env.EXECUTE === 'true') {
      console.log('⚠️  EXECUTE=true detected, performing real swap on devnet...\n');
